...
Starling treats the event log as the source of truth. The runtime, the
inspector, and replay verification all read the same shape. Once you accept
that, every other design choice falls out.
...
`step` helpers append a `SideEffectRecorded` event on the live run. Replay
reads that event and returns the recorded value without invoking the
closure. This is what makes a run portable — the recording carries every
non-deterministic value alongside the agent's deterministic logic.
...
All three implement the same `EventLog` interface and the same migration
contract.

## What's intentionally out of scope

The plan calls these out and Starling doesn't ship them:

- Prompt templating language
- Visual / no-code builder
- Generic chain or graph DSL
- Built-in vector database
- Hosted Starling service
- Large bundled tool registry in core
- Built-in memory store

You write Go. The runtime stays small. The plan is to win on a narrow
wedge — production debugging via replay — not on breadth.
