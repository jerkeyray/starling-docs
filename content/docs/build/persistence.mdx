...
All three built-in backends satisfy `RunLister` and `RunPageLister`.
The inspector uses `RunPageLister` when available, and falls back to
`RunLister` for custom backends that only implement the older listing
interface.
