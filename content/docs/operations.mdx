...
Everything an operator needs in one page: process model, backend choice,
schema migrations, security posture, retention, metrics, tracing, and
recovery.
...
Event payloads can contain entire user/assistant conversations,
reasoning, and tool I/O. Treat the database file or table as PII:

- Encrypt at rest (filesystem-level for SQLite; `pgcrypto` / TDE for
  Postgres).
- Restrict OS-level access; SQLite file `0600` owned by the agent user.
- The inspector serves payloads verbatim: never expose past a trusted
  operator audience.

There is no built-in field-level redaction. Redact at the tool boundary
if specific values must stay out of the log.

## Retention

The event log is append-only by design. Mutating an event breaks the
hash chain for every later event in the same run.

### What you cannot do

- **Mutate events.** Hash chain integrity depends on every committed
  event being immutable.
- **Delete a single event from a run.** Same reason. The unit of
  deletion is the entire run.
- **Reuse a `run_id`.** Once a run is recorded, its id is retired.

### What you can do

- **Delete whole runs.** Removing all events for a `run_id` is safe.
- **Archive whole runs to cold storage.** Export to NDJSON via
  `starling export`, store the file somewhere cheaper, then delete.
- **Partition by time.** Postgres only: drops to `DROP PARTITION` for
  retention. Use `pg_partman` or a cron job to roll new monthly
  partitions; drop the oldest at the retention horizon.
- **Truncate the inspector view.** `RunSummary.StartedAt` lets you
  filter to recent runs without deletion.
...
| `starling_provider_call_seconds`                | Histogram | `model`                           |
| `starling_provider_tokens_total`                | Counter   | `model`, `type`                   |
| `starling_tool_call_seconds`                    | Histogram | `tool`                            |
| `starling_tool_call_total`                      | Counter   | `tool`, `status`, `error_type`    |
| `starling_eventlog_append_seconds`              | Histogram | `kind`, `status`                  |
...
- [ ] Metrics scraped; dashboards alert on
      `starling_eventlog_append_seconds` p99 and
      `starling_provider_call_seconds` p99.
